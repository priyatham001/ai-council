...
export class AIOrchestrator {
  ...
  public async runCouncilPipeline(
    ...
    const isDemo = isDemoRequested || configuredProviders.length === 0;
    ...
    if (isDemoRequested || configuredProviders.length === 0) {
      ...
      // Generate demo responses for all 4 models
      responses = getDemoCouncilResponses(question, mode);
      onProgress?.('Collecting independent demo responses', 3);
    } else {
      // Real provider execution: ALL configured providers participate
      ...
      onProgress?.(`Consulting ${targetProviders.length} active AI provider(s) in parallel`, 2);

      // Concurrent independent execution with Promise.allSettled (fault-tolerant)
      ...
      const targetResponses: ProviderResponse[] = settledResults.map((res, idx) => {
        ...
        if (res.status === 'fulfilled') {
          ...
          return {
            ...
            error: (res.reason as Error)?.message || 'Provider execution failed.',
          };
        }
      });

      // Record any unconfigured providers so the UI and history accurately reflect all 4 providers
      const unconfiguredProviders = allProviders.filter(
        (p) => !targetProviders.some((tp) => tp.id === p.id)
      );
      const unconfiguredResponses: ProviderResponse[] = unconfiguredProviders.map((p) => ({
        provider: p.id,
        providerName: p.name,
        model: p.modelName,
        answer: '',
        responseTime: 0,
        status: 'error' as const,
        error: `${p.name} is not configured (API key not set in environment).`,
      }));

      responses = [...targetResponses, ...unconfiguredResponses];
    ...
    const [analysis, critic] = await Promise.all([
      runCrossAnalysis(question, responses, mode, isDemo),
    ...
    const decision = await runFinalJudge(question, responses, analysis, critic, mode, isDemo);
